...
export default function LicensedPartners() {
  ...
  return (
    <div ref={containerRef} className="mt-24 pt-20 border-t border-navy/5">
      <div className="text-[10px] uppercase tracking-[0.4em] text-navy/40 font-black mb-12 text-center">Licensed Partners</div>
      
      {/* Horizontal Flex Layout as requested before */}
      <div className="flex flex-row flex-wrap justify-center items-center gap-12 md:gap-20">
        {partners.map((p) => (
          <div key={p.name} className="partner-logo group flex flex-col items-center gap-3">
             <div className="relative w-16 md:w-20 aspect-square flex items-center justify-center">
                <img 
                  src={p.img} 
                  alt={p.name} 
             ...
             <span className="text-[9px] font-black uppercase tracking-widest text-navy/20 group-hover:text-navy transition-colors duration-500">
               {p.name}
             </span>
          </div>
        ))}
